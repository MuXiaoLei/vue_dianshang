  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address,index) in addressList" :key="index">
        ...
        <p @click="changDefault(address,addressList)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(trade,index) in tradeList.detailArrayList" :key="index">
          <li>
            <img :src="trade.imgUrl" alt="" style="width:50px">
          </li>
          <li>
            <p>{{trade.skuName}}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{UserAddressIsDefault.fullAddress}}</span>
        收货人：<span>{{UserAddressIsDefault.consignee}}</span>
        <span>{{UserAddressIsDefault.phoneNum}}</span>
      </div>
    </div>
    <div class="sub clearFix">
        <button class="subBtn" @click="subBtn">提交订单</button>
      <!-- <router-link class="subBtn" to="/pay" @click="subBtn">提交订单</router-link> -->
    </div>
  </div>
</template>
...
import {mapState} from 'vuex'
  export default {
    ...
    computed:{
        ...mapState({
            addressList:state=>state.trade.UserAddress,
    ...
    methods: {
        changDefault(address,addressList){
        ...
        async subBtn(){
            try {
                ...
                let data = {
                    "consignee": this.UserAddressIsDefault.consignee,
                    "consigneeTel": this.UserAddressIsDefault.phoneNum,
                    "deliveryAddress": this.UserAddressIsDefault.fullAddress,
                    "paymentWay": "ONLINE",
                    "orderComment": this.msg,
                    "orderDetailList":this.tradeList.detailArrayList
                };
                await this.$store.dispatch('submitPay',{tradeNo,data});
